...
describe('login', () => {
  ...
  it('should authenticate user with valid credentials', async () => {
    // Create test user with properly hashed password using Bun.password.hash
    const hashedPassword = await Bun.password.hash(loginInput.password);
  ...
  it('should verify password against Bun.password.hash correctly', async () => {
    const testPassword = 'mySecurePassword123!';
    
    // Create user with password hashed using Bun.password.hash
    ...
    const isValid = await Bun.password.verify(testPassword, hashedPassword);
    ...
    const isWrongValid = await Bun.password.verify(wrongPassword, hashedPassword);
